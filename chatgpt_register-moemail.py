...
class ChatGPTRegister:
    ...
    def _create_duckmail_session(self):
        ...
        return session
    def _get_moemail_domains(self):
        """获取可用邮箱域名列表"""
        api_base = MOEMAIL_API_BASE.rstrip("/")
        session = self._create_duckmail_session()
        res = session.get(f"{api_base}/api/config", timeout=15, impersonate="chrome131")
        if res.status_code != 200:
            raise Exception(f"获取 Moemail 配置失败: {res.status_code} - {res.text[:200]}")
        data = res.json()
        domains_str = data.get("emailDomains", "")
        domains = [d.strip() for d in domains_str.split(",") if d.strip()]
        if not domains:
            raise Exception("未获取到可用域名")
        return domains
    def create_temp_email(self):
        """创建 Moemail 临时邮箱，返回 (email, email_id)"""
        if not MOEMAIL_API_KEY:
            raise Exception("MOEMAIL_API_KEY 未设置，无法创建临时邮箱")
        # 生成随机邮箱前缀 8-13 位
        chars = string.ascii_lowercase + string.digits
        length = random.randint(8, 13)
        email_local = "".join(random.choice(chars) for _ in range(length))
        # 获取域名并随机选择
        domains = self._get_moemail_domains()
        ...
        if not email or not email_id:
            raise Exception("创建邮箱返回数据不完整")
        return email, email_id
    def _fetch_emails_moemail(self, email_id: str):
        """获取指定邮箱的邮件列表"""
        try:
            api_base = MOEMAIL_API_BASE.rstrip("/")
            session = self._create_duckmail_session()
            ...
            return []
    def _fetch_email_detail_moemail(self, email_id: str, msg_id: str):
        """获取指定邮件详情"""
        try:
            api_base = MOEMAIL_API_BASE.rstrip("/")
            session = self._create_duckmail_session()
            res = session.get(
                f"{api_base}/api/emails/{email_id}/{msg_id}",
                timeout=15,
                impersonate="chrome131"
            )
            if res.status_code == 200:
    ...
    def _extract_verification_code(self, email_content: str):
        ...
        for pattern in patterns:
            ...
            for code in matches:
                ...
                return code
        return None
    def wait_for_verification_email(self, email_id: str, timeout: int = 120):
        """等待并提取验证码"""
        start_time = time.time()
        while time.time() - start_time < timeout:
            messages = self._fetch_emails_moemail(email_id)
            if messages:
                # 最新邮件
                first_msg = messages[0]
                msg_id = first_msg.get("id")
                if msg_id:
                    detail = self._fetch_email_detail_moemail(email_id, msg_id)
                    if detail:
                        content = detail.get("content") or detail.get("html") or ""
    ...
    def perform_codex_oauth_login_http(self, email: str, password: str, mail_token: str = None):
        ...
        if need_oauth_otp:
            ...
            while time.time() < otp_deadline and not otp_success:
                messages = self._fetch_emails_moemail(mail_token) or []
                candidate_codes = []
                ...
                for msg in messages[:12]:
                    ...
                    if not msg_id:
                        continue
                    detail = self._fetch_email_detail_moemail(mail_token, msg_id)
                    if not detail:
                        continue
                    content = detail.get("text") or detail.get("html") or ""
                    code = self._extract_verification_code(content)
