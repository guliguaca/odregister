...
ENABLE_OAUTH = _CONFIG["enable_oauth"]
OAUTH_REQUIRED = _CONFIG["oauth_required"]
...
def _save_codex_tokens(email: str, tokens: dict):
    ...
    # 保存 AK/RK (可选，CI 环境下通常不需要持久化保存 txt，但为了兼容性保留)
    if access_token:
        with _file_lock:
    ...
    if UPLOAD_API_URL:
        upload_success = _upload_token_json(token_path)
    
    # 【安全调整】上传成功后删除本地 JSON 文件
    if upload_success and os.path.exists(token_path):
        try:
            os.remove(token_path)
            with _print_lock:
                print(f"  [Security] 本地 Token JSON 已删除：{token_path}")
        except Exception as e:
            with _print_lock:
                print(f"  [Warning] 删除本地 Token JSON 失败：{e}")
...
def _register_one(idx, total, proxy, output_file):
    ...
    try:
        ...
        with _print_lock:
            ...
            # traceback.print_exc() # CI 环境下可减少 traceback 打印以防泄露
...
def run_batch(total_accounts: int = 3, output_file="registered_accounts.txt",
    ...
    print(f"\n{'#'*60} ")
    print(f"  ChatGPT 批量自动注册 (GitHub Action 版) ")
    print(f"  注册数量：{total_accounts} | 并发数：{actual_workers} ")
    print(f"  代理状态：{'启用 (' + proxy + ')' if proxy else '禁用'} ")
    print(f"{'#'*60}\n ")
...
def main():
    print("=" * 60)
    print("  ChatGPT 批量自动注册工具 (GitHub Action 适配版)")
    print("=" * 60)
